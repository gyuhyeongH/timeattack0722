...
class ApplyView(APIView):
    ...
    def get(self,request):
        jobpostactivity = JobPostActivity.objects.filter(applystate__state=request.data.get('status'))

        if jobpostactivity.exists():
            apply_serialzer = JobPostActivitySerializer(jobpostactivity, many=True).data

            return Response(apply_serialzer ,status=status.HTTP_200_OK)
